    ...
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.88)', zIndex: 400, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div onClick={e => e.stopPropagation()} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14, maxHeight: '90vh', overflowY: 'auto', padding: '20px 10px' }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          {onSearchCommunity && <button onClick={onSearchCommunity} style={{ background: 'rgba(139,92,246,0.1)', border: '1px solid rgba(139,92,246,0.3)', borderRadius: 8, color: '#a78bfa', fontSize: 12, fontWeight: 600, padding: '7px 14px', cursor: 'pointer', fontFamily: 'inherit' }}>Search Community</button>}
          {onSearchMarketplace && <button onClick={onSearchMarketplace} style={{ background: 'rgba(47,125,163,0.1)', border: '1px solid rgba(47,125,163,0.3)', borderRadius: 8, color: '#52a9cd', fontSize: 12, fontWeight: 600, padding: '7px 14px', cursor: 'pointer', fontFamily: 'inherit' }}>Search Marketplace</button>}
        </div>
