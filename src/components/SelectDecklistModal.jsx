...
function LeaderCard({ deck, onClick }) {
  ...
  return (
    <div
      ...
      style={{ background: 'rgba(140,176,208,0.05)', border: '1px solid rgba(140,176,208,0.07)', borderRadius: 14, overflow: 'hidden', cursor: 'pointer', transition: 'all 0.15s' }}
    ...
    >
      <div style={{ position: 'relative', height: 120, background: 'rgba(140,176,208,0.03)', overflow: 'hidden' }}>
        {!errored ? (
      ...
      <div style={{ padding: '10px 12px' }}>
        <div style={{ fontSize: 13, fontWeight: 700, color: '#e9f1f8', marginBottom: 2 }}>{deck.name}</div>
        <div style={{ fontSize: 11, color: '#9db2c6' }}>{deck.leader_name} · {deck.leader_id}</div>
      </div>
    </div>
  )
}
    ...
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 300, display: 'flex', alignItems: isMobile ? 'flex-end' : 'center', justifyContent: 'center', padding: isMobile ? 0 : 20 }}>
      <div onClick={e => e.stopPropagation()} style={modalBox}>
        ...
        <div style={{ padding: '18px 24px', borderBottom: '1px solid rgba(140,176,208,0.07)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
          <div>
            <div style={{ fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '1.6px', color: '#dcb35e', marginBottom: 2 }}>Builds</div>
            <div style={{ fontSize: 16, fontWeight: 700, color: '#e9f1f8', letterSpacing: '-0.3px' }}>Select a Decklist</div>
          </div>
          <button onClick={onClose} style={{ background: 'rgba(0,0,0,0.5)', border: '1px solid rgba(140,176,208,0.15)', borderRadius: 6, color: '#e9f1f8', fontSize: 16, width: 30, height: 30, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
        </div>

        {/* Search */}
        <div style={{ padding: '12px 24px', borderBottom: '1px solid rgba(140,176,208,0.07)', flexShrink: 0 }}>
          <input type="text" placeholder="Search by leader or name..." value={search} onChange={e => setSearch(e.target.value)} style={{ width: '100%', background: 'rgba(140,176,208,0.05)', border: '1px solid rgba(140,176,208,0.07)', borderRadius: 8, padding: '8px 12px', color: '#e9f1f8', fontSize: 13, outline: 'none', fontFamily: 'inherit', boxSizing: 'border-box' }} />
        </div>
        ...
        <div style={{ overflowY: 'auto', flex: 1, padding: 16 }}>
          {loading ? (
            ...
            <div style={{ textAlign: 'center', padding: '60px 20px', color: '#67809a' }}>
              ...
              <div style={{ fontSize: 14, fontWeight: 600, color: '#9db2c6', marginBottom: 4 }}>{search ? 'No decklists match your search.' : 'No saved decklists found.'}</div>
