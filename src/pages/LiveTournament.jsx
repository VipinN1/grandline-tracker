  ...
  return (
    <div style={{ maxWidth: 560, margin: '0 auto' }}>
      <div style={{ marginBottom: '1.5rem' }}>
        ...
        <div style={{ fontSize: 13, color: '#6b7a99' }}>Track your rounds in real time</div>
      </div>

      <div style={{ background: '#161b27', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, padding: 24, display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div ref={storeRef} style={{ position: 'relative' }}>
          ...
          {selectedStore ? (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#1c2333', border: '1px solid #3d7fff44', borderRadius: 8, padding: '9px 12px' }}>
              ...
              <button onClick={() => setSelectedStore(null)} style={{ background: 'none', border: 'none', color: '#6b7a99', cursor: 'pointer', fontSize: 16, padding: 0 }}>✕</button>
            </div>
          ) : (
            <>
              ...
              {storeOpen && (
                <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 50, background: '#1c2333', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, marginTop: 4, boxShadow: '0 8px 24px rgba(0,0,0,0.4)', maxHeight: 200, overflowY: 'auto' }}>
                  {filteredStores.map(s => (
                    <div key={s.id} onClick={() => { setSelectedStore(s); setStoreQuery(''); setStoreOpen(false) }} style={{ padding: '10px 14px', cursor: 'pointer', fontSize: 13, color: '#f0f2f5', borderBottom: '1px solid rgba(255,255,255,0.05)' }} onMouseEnter={e => e.currentTarget.style.background = 'rgba(255,255,255,0.05)'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
                      {s.name} {s.city && <span style={{ color: '#6b7a99' }}>· {s.city}</span>}
                    </div>
                  ))}
                  {storeQuery && <div onClick={() => { setStoreOpen(false) }} style={{ padding: '10px 14px', cursor: 'pointer', fontSize: 13, color: '#3d7fff', fontWeight: 600 }}>+ Use "{storeQuery}" as location</div>}
                </div>
              )}
            </>
          )}
        ...
        <div ref={seriesRef} style={{ position: 'relative' }}>
          ...
          {selectedSeries ? (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#1c2333', border: '1px solid #3d7fff44', borderRadius: 8, padding: '9px 12px' }}>
              ...
              <button onClick={() => setSelectedSeries(null)} style={{ background: 'none', border: 'none', color: '#6b7a99', cursor: 'pointer', fontSize: 16, padding: 0 }}>✕</button>
            </div>
          ) : (
            <>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <div>
            <label style={labelStyle}>Tournament Name {selectedSeries && <span style={{ color: '#3a4560', fontWeight: 400 }}>(auto)</span>}</label>
        ...
        <button onClick={handleStart} disabled={saving} style={{ width: '100%', padding: 12, borderRadius: 8, border: 'none', background: saving ? '#2a4a8a' : '#34d399', color: '#0f1117', fontSize: 14, fontWeight: 700, cursor: saving ? 'default' : 'pointer', fontFamily: 'inherit' }}>
  ...
  return (
    <div style={{ background: '#161b27', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, padding: 20 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <button onClick={logRound} disabled={saving} style={{ width: '100%', padding: 11, borderRadius: 8, border: 'none', background: saving ? '#2a4a8a' : '#3d7fff', color: '#fff', fontSize: 14, fontWeight: 700, cursor: saving ? 'default' : 'pointer', fontFamily: 'inherit' }}>
...
function RoundHistory({ rounds }) {
  ...
  return (
    <div style={{ background: '#161b27', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, padding: 20 }}>
      <div style={{ fontSize: 13, fontWeight: 700, color: '#6b7a99', textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: 14 }}>Round History</div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {rounds.map(r => (
          <div key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 12, background: '#1c2333', borderRadius: 10, padding: '10px 14px' }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: '#6b7a99', minWidth: 60 }}>R{r.round_number}</div>
            ...
            {r.opponent_leader_id && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, flex: 1 }}>
                ...
                <div style={{ fontSize: 12, color: COLORS[r.opponent_leader_color] ?? '#6b7a99' }}>{r.opponent_leader_name}</div>
              </div>
            )}
            {!r.opponent_leader_id && <div style={{ flex: 1, fontSize: 12, color: '#3a4560' }}>Unknown leader</div>}
...
function ActiveTournament({ tournament, session, onFinish }) {
  ...
  return (
    <div>
      {/* Header */}
      <div style={{ background: '#161b27', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, padding: 20, marginBottom: 14 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 14 }}>
          <img src={getCardImageUrl(tournament.leader_id)} alt={tournament.leader_name} style={{ width: 48, height: 66, objectFit: 'cover', objectPosition: 'top', borderRadius: 6, border: `2px solid ${COLORS[tournament.leader_color] ?? '#3d7fff'}` }} onError={e => { e.target.style.display = 'none' }} />
          <div style={{ flex: 1 }}>
      ...
      {showFinishConfirm && (
        <div onClick={() => setShowFinishConfirm(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div onClick={e => e.stopPropagation()} style={{ background: '#161b27', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 14, padding: 24, width: 360 }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => setShowFinishConfirm(false)} style={{ flex: 1, padding: 10, borderRadius: 8, border: '1px solid rgba(255,255,255,0.1)', background: 'transparent', color: '#6b7a99', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>Cancel</button>
              <button
                onClick={async () => {
