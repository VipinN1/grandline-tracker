...
export default function UserProfilePage({ session }) {
  ...
  function FriendButton() {
    ...
    if (friendStatus === 'accepted') return (
      <button onClick={removeFriend} disabled={friendLoading} style={{ fontSize: 12, fontWeight: 600, padding: '5px 14px', borderRadius: 8, border: '1px solid rgba(240,82,82,0.3)', background: 'rgba(240,82,82,0.08)', color: '#f05252', cursor: 'pointer', fontFamily: 'inherit', marginTop: 10 }}>Remove Friend</button>
    )
    if (friendStatus === 'pending_sent') return (
      <button disabled style={{ fontSize: 12, fontWeight: 600, padding: '5px 14px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.1)', background: 'transparent', color: '#7c6fa0', cursor: 'default', fontFamily: 'inherit', marginTop: 10 }}>Request Sent</button>
    )
    if (friendStatus === 'pending_received') return (
      <button onClick={acceptRequest} disabled={friendLoading} style={{ fontSize: 12, fontWeight: 600, padding: '5px 14px', borderRadius: 8, border: 'none', background: '#34d399', color: '#0f1117', cursor: 'pointer', fontFamily: 'inherit', marginTop: 10 }}>Accept Request</button>
    )
    return (
      <button onClick={sendFriendRequest} disabled={friendLoading} style={{ fontSize: 12, fontWeight: 600, padding: '5px 14px', borderRadius: 8, border: 'none', background: 'linear-gradient(135deg, #7c3aed, #a855f7)', color: '#fff', cursor: 'pointer', fontFamily: 'inherit', marginTop: 10 }}>+ Add Friend</button>
    )
  }
  ...
  if (!profile) {
    return (
      <div style={{ textAlign: 'center', padding: '80px 20px' }}>
        <div style={{ fontSize: 15, fontWeight: 600, color: '#7c6fa0', marginBottom: 8 }}>Profile not found</div>
        <button onClick={() => navigate(-1)} style={{ color: '#8b5cf6', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit', fontSize: 13 }}>← Go back</button>
      </div>
    )
  }
  ...
  return (
    <div>
      <div style={{ marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: 12 }}>
        <button
          onClick={() => navigate(-1)}
          style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, color: '#7c6fa0', fontSize: 13, padding: '6px 12px', cursor: 'pointer', fontFamily: 'inherit', flexShrink: 0 }}
        >
          ← Back
        </button>
        <div>
          <div style={{ fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '1.2px', color: '#8b5cf6', marginBottom: 2 }}>Player</div>
          <div style={{ fontSize: 20, fontWeight: 700, color: '#f0f2f5', letterSpacing: '-0.4px' }}>{profile.username}</div>
        </div>
      </div>
      ...
      <div style={{ background: 'rgba(139,92,246,0.05)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, padding: isMobile ? 16 : 24, marginBottom: 14 }}>
        <div style={{ display: 'flex', alignItems: isMobile ? 'flex-start' : 'center', gap: isMobile ? 14 : 20 }}>
          <div style={{ width: 64, height: 64, borderRadius: 14, background: profile.avatar_url ? 'transparent' : '#8b5cf6', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, fontWeight: 700, color: '#fff', flexShrink: 0, overflow: 'hidden', border: '2px solid rgba(255,255,255,0.1)' }}>
            {profile.avatar_url
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
              <div style={{ fontSize: isMobile ? 17 : 20, fontWeight: 700, color: '#f0f2f5', letterSpacing: '-0.3px' }}>{profile.username}</div>
              {profile.pronouns && <div style={{ fontSize: isMobile ? 11 : 12, color: '#7c6fa0' }}>Pronouns: <span style={{ color: '#a78bfa' }}>{profile.pronouns}</span></div>}
            </div>
        ...
        {profile.bio && (
          <div style={{ marginTop: 16, paddingTop: 14, borderTop: '1px solid rgba(255,255,255,0.05)' }}>
            ...
            <div style={{ fontSize: 13, color: '#f0f2f5', lineHeight: 1.6 }}>{profile.bio}</div>
          </div>
        )}
      </div>
