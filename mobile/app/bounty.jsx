...
export default function BountyBoard() {
  ...
  useEffect(() => {
    async function load() {
      const [{ data }, { data: simTours }, { data: simMatches }] = await Promise.all([
        supabase
          .from('tournaments')
          .select('id, user_id, name, date, placement, wins, losses, leader_id, leader_name, leader_color, is_legacy, profiles(id, username, avatar_url, location, bio)')
      ...
      let profileMap = {}
      if (missing.length > 0) {
        const { data: profs } = await supabase.from('profiles').select('id, username, avatar_url, location, bio').in('id', missing)
  ...
  tournaments.forEach(t => {
    ...
    // Result", web-only) still count toward event totals and win rate, but shouldn't
    // be able to buy bounty — only wins actually tracked here contribute to bounty.
  ...
  Object.entries(simPerTour).forEach(([uid, tours]) => {
    if (!playerMap[uid]) {
      ...
      playerMap[uid] = { user_id: uid, profile: prof ?? { id: uid, username: 'Unknown', avatar_url: null }, bounty: 0, wins: 0, losses: 0, tournaments: 0, leaders: {} }
    }
        ...
        ListHeaderComponent={
          <View style={{ gap: 14, marginBottom: 14 }}>
            ...
            <View style={{ backgroundColor: 'rgba(200,162,74,0.04)', borderWidth: 1, borderColor: 'rgba(200,162,74,0.15)', borderRadius: 12, padding: 12 }}>
              ...
              <Text style={{ fontSize: 10, color: colors.faint, fontFamily: font.body, marginTop: 8 }}>
